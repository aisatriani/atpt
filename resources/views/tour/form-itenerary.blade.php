            ...
            <div class="col-lg-12">
                ...
                <div class="panel panel-default">
                    <div class="panel-heading">
                        <div class="pull-left">
                            <h4>Daftar Itenerary {{ $tour->nama_tour }}</h4>
                        </div>
                    ...
                    <div class="panel-body">
                        <div class="row">
                            <div class="col-lg-10">
                                ...
                                <form action="{{route('tour.itenerary.store',[$tour->id])}}" method="post" class="form-horizontal" role="form">

                                    {{ csrf_field() }}
                                    @if($tour->itenerary !== null)
                                        @for($i = 0; $i < count($tour->itenerary); $i++)
                                    {{--@foreach($tour->itenerary as $itenerary)--}}

                                        <div class="form-group">
                                            <label for="itenerary[]" class="col-sm-2 control-label">Item</label>
                                            <div class="col-sm-6">
                                                <input type="text" name="itenerary[]" class="form-control" id="itenerary[]" placeholder="Item Itenerary" value="{{ $tour->itenerary[$i] }}">
                                            </div>
...
@push('js')
    <script>
        $(document).ready(function () {
            var no = '{{ count($tour->itenerary) }}';
            $('#add-item').click(function (e) {
                var content = $('#content-form');
                var html = '<div class="form-group">'+
                            '<label for="itenerary[]" class="col-sm-2 control-label">item</label>'+
                            '<div class="col-sm-6">'+
                            '<input type="text" name="itenerary[]" class="form-control" id="itenerary" placeholder="Item Itenerary">'+
                            '</div>'+
                            '<div id="button-remove" class="col-sm-2">'+
                            '<button type="button" id="remove-item" class="btn btn-danger btn-circle btn-xs"><i class="fa fa-times"></i>'+
                            '</button> Hapus'+
